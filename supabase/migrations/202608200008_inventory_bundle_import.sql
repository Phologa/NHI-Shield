-- Phase 2: one-file inventory import with external-key resolution and provenance.
alter table public.import_runs drop constraint import_runs_entity_type_check;
alter table public.import_runs add constraint import_runs_entity_type_check check (entity_type in ('security_inventory','machine_identities','credentials','resources','access_relationships'));
alter table public.import_runs add column analysis_status text not null default 'pending' check (analysis_status in ('pending','ready','failed'));
alter table public.credentials add column provenance text not null default 'observed' check (provenance in ('observed','derived','user_confirmed'));
alter table public.resources add column provenance text not null default 'observed' check (provenance in ('observed','derived','user_confirmed'));
alter table public.access_relationships add column provenance text not null default 'observed' check (provenance in ('observed','derived','user_confirmed'));
alter table public.machine_identities add column ownership_conflict boolean not null default false;

create or replace function public.import_security_inventory_csv(import_rows jsonb)
returns jsonb language plpgsql security invoker set search_path = public as $$
declare item jsonb; target_org uuid; identity_id uuid; resource_id uuid; processed integer := 0; relationship_count integer := 0; credential_count integer := 0;
begin
  ...
  for item in select value from jsonb_array_elements(import_rows) loop
    ...
    if nullif(item->>'resource_external_id','') is not null then
      ...
      values(target_org,identity_id,resource_id,item->>'access_level',(item->>'access_privileged')::boolean,'csv_import','observed',now())
    ...
    if nullif(item->>'credential_label','') is not null then
      if exists(select 1 from public.credentials where organisation_id=target_org and machine_identity_id=identity_id and label=item->>'credential_label' and credential_type=(item->>'credential_type')::public.credential_type) then
        update public.credentials set status=(item->>'credential_status')::public.credential_status,last_rotated_at=nullif(item->>'credential_last_rotated_at','')::timestamptz,expires_at=nullif(item->>'credential_expires_at','')::timestamptz,provenance='observed' where organisation_id=target_org and machine_identity_id=identity_id and label=item->>'credential_label' and credential_type=(item->>'credential_type')::public.credential_type;
      else
        ...
        values(target_org,identity_id,(item->>'credential_type')::public.credential_type,item->>'credential_label',(item->>'credential_status')::public.credential_status,nullif(item->>'credential_last_rotated_at','')::timestamptz,nullif(item->>'credential_expires_at','')::timestamptz,'observed');
      end if;
