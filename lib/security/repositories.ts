...
export async function getOverviewCounts(context: SecurityContext) {
  const [identities, credentials, resources, findings, incidents, activity] = await Promise.all([
  ...
  if (identities.error || credentials.error || resources.error || findings.error || incidents.error || activity.error) throw new Error("OVERVIEW_READ_FAILED");
  const identityRows = identities.data ?? []; const findingRows = findings.data ?? [];
  const incidentRows = incidents.data ?? [];
  return { identities: identities.count ?? identityRows.length, credentials: credentials.count ?? 0, resources: resources.count ?? 0, activeFindings: findingRows.length, criticalHighFindings: findingRows.filter((row) => row.severity === "critical" || row.severity === "high").length, identitiesWithoutOwners: identityRows.filter((row) => !row.owner_name).length, privilegedIdentities: identityRows.filter((row) => row.privilege_level === "high" || row.privilege_level === "critical").length, activeIncidents: incidentRows.length, highCriticalIncidents: incidentRows.filter((row) => row.severity === "critical" || row.severity === "high").length, activityEvents: activity.count ?? 0 };
}