...
export async function getOverviewCounts(context: SecurityContext) {
  const [identities, credentials, resources, relationships, findings, incidents, activity] = await Promise.all([
    ...
    context.supabase.from("access_relationships").select("id, privileged", { count: "exact", head: false }).eq("organisation_id", context.organisationId),
    ...
    context.supabase.from("activity_events").select("id", { count: "exact", head: true }).eq("organisation_id", context.organisationId),
  ]);
  if (identities.error || credentials.error || resources.error || relationships.error || findings.error || incidents.error || activity.error) throw new Error("OVERVIEW_READ_FAILED");
  ...
  return { identities: identities.count ?? identityRows.length, credentials: credentials.count ?? 0, resources: resources.count ?? 0, relationships: relationships.count ?? relationshipRows.length, privilegedRelationships: relationshipRows.filter((row) => row.privileged).length, activeFindings: findingRows.length, criticalHighFindings: findingRows.filter((row) => row.severity === "critical" || row.severity === "high").length, identitiesWithoutOwners: identityRows.filter((row) => !row.owner_name).length, privilegedIdentities: identityRows.filter((row) => row.privilege_level === "high" || row.privilege_level === "critical").length, activeIncidents: incidentRows.length, highCriticalIncidents: incidentRows.filter((row) => row.severity === "critical" || row.severity === "high").length, activityEvents: activity.count ?? 0 };
}
