...
type RelatedName = Array<{ name: string }> | null;
...
export default async function IncidentsPage() {
  ...
  return <section>
    <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end"><div><p className="font-sans text-sm font-semibold uppercase tracking-wider text-[var(--teal)]">Runtime foundation</p><h2 className="mt-3 font-serif text-4xl">Incidents</h2><p className="font-sans mt-4 max-w-2xl text-sm leading-6 text-[var(--muted)]">Manual activity input and deterministic incident detection. No AI analysis or remediation is active.</p></div><DetectionButton /></div>
    {!incidents.length ? <div className="mt-8 border border-[var(--line)] bg-white p-8"><h3 className="font-serif text-xl">No incidents recorded</h3><p className="font-sans mt-3 text-sm leading-6 text-[var(--muted)]">Record verified manual activity, then run detection. NHI Shield does not fabricate incidents.</p></div> : <div className="mt-8 space-y-3">{incidents.map((incident) => <a className="focus-ring block border border-[var(--line)] bg-white p-5 font-sans transition-colors hover:border-[var(--teal)]" href={`/incidents/${incident.id}`} key={incident.id}><div className="flex flex-wrap justify-between gap-3"><strong>{incident.title}</strong><span className="uppercase tracking-wider">{incident.severity} · {incident.status}</span></div><p className="mt-2 text-sm text-[var(--muted)]">{incident.machine_identities?.[0]?.name ?? "Identity not recorded"} → {incident.resources?.[0]?.name ?? "Resource not recorded"}</p><p className="mt-2 text-xs text-[var(--muted)]">Last activity {new Date(incident.last_activity_at).toLocaleString()}</p></a>)}</div>}
    <details className="mt-10 border border-[var(--line)] bg-[#eef4f7] p-6"><summary className="cursor-pointer font-serif text-xl">Record manual activity</summary>{!identityOptions.length ? <p className="font-sans mt-4 text-sm text-[var(--muted)]">Register a machine identity before recording activity.</p> : <div className="mt-6"><ActivityForm identities={identityOptions} resources={resourceOptions} /></div>}</details>
    <div className="mt-10"><h3 className="font-serif text-2xl">Recent activity</h3>{!events.length ? <p className="font-sans mt-4 border border-[var(--line)] bg-white p-6 text-sm text-[var(--muted)]">No activity events recorded.</p> : <div className="mt-4 space-y-2">{events.map((event) => <div className="border border-[var(--line)] bg-white p-4 font-sans text-sm" key={event.id}><strong>{event.action}</strong><span className="ml-3 text-[var(--muted)]">{event.outcome}</span><p className="mt-1 text-xs text-[var(--muted)]">{event.machine_identities?.[0]?.name ?? "Identity"} · {event.resources?.[0]?.name ?? "No resource"} · {new Date(event.occurred_at).toLocaleString()}</p></div>)}</div>}</div>
  </section>;
}
